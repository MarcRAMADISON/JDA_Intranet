...
export async function getData({
  ...
  if (filters?.statut && filters?.statut !== 'TOUT' && !isAll) {
    rows = await fetch(
      `${process.env.PUBLIC_URL}/api/fiches${query}filters[statut][$eq]=${
        filters.statut
    ...
    rows = await fetch(
      `${process.env.PUBLIC_URL}/api/fiches${query}populate=user,venduePar,userAssigne&pagination[limit]=999`,
      {
        method: "GET",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
    ...
    rows = await fetch(
      `${process.env.PUBLIC_URL}/api/fiches${query}filters[statut][$eq]=${filters?.statut}&populate=user,venduePar,userAssigne&pagination[limit]=999`,
      {
        method: "GET",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
    ...
    rows = await fetch(
      `${process.env.PUBLIC_URL}/api/fiches${query}pagination[start]=${
        start || 0
...
export const getUsers = async () => {
  ...
  const idEquipe = Cookies.get("idEquipe");
    ...
    return fetch(
      `${process.env.PUBLIC_URL}/api/users?filters[equipe]=${idEquipe}&fields[0]=username&fields[1]=type`,
      {
        method: "GET",
        headers: {
          "Content-type": "application/json; charset=UTF-8",
